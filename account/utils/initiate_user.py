# utils/initiate_user.py ===> postgresql version
import io
import os
import sys
import re
...
def initiate_bulk_user(file) -> Dict[str, Any]:
    ...
    - owner is set to 'admin' user if present (since view cannot pass request.user).
    - All users passwords are set to DEFAULT_PASSWORD (overwrites existing).
    - Keeps insertion order same as Excel row order.
    - IMPORTANT CHANGE:
      ✅ Removed manual PK(id) assignment (DB auto-increment handles it).
    """
    ...
    for _, row in df.iterrows():
        ...
        if d2:
            if not division_obj:
                return {"success": False, "message": f"Group을 지정하려면 Division도 필요 (user={uname})"}
            grp_name = _fmt_grp(d1, d2)
            grp_defaults = {"division_parent": division_obj}
            if owner:
                grp_defaults["owner"] = owner
            group_obj, _ = Group.objects.get_or_create(
                group_name=grp_name,
        ...
        if d3:
            ...
            if not created_team:
                changed = False
                if getattr(team_obj, "division_parent_id", None) != (division_obj.id if division_obj else None):
                    team_obj.division_parent = division_obj
                    changed = True
                if getattr(team_obj, "group_parent_id", None) != (group_obj.id if group_obj else None):
                    team_obj.group_parent = group_obj
                    changed = True
                if not (getattr(team_obj, "code", "") or "").strip():
                    team_obj.code = team_code
                    changed = True
                if not getattr(team_obj, "batch_no", None):
                    team_obj.batch_no = batch_no
                    changed = True
                if owner and not getattr(team_obj, "owner_id", None):
                    team_obj.owner = owner
                    changed = True
                if changed:
                    team_obj.save(update_fields=["division_parent", "group_parent", "code", "batch_no", "owner"])

        # User upsert (create if missing; set owner if available)
        user_defaults = {}
        if owner:
            user_defaults["owner"] = owner
        ...
        # 저장(필요 필드만 업데이트해도 되고, 그냥 save()도 OK)
        user.save()
